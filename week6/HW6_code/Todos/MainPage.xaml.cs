...
namespace Todos
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void insertToDb(int id, string title, string description, string date)
        {
            //var db = App.conn;
            try
            {
        ...
        async private void getItemFromDb(object sender, RoutedEventArgs e)
        {
            string searchInfo = search.Text;
            ...
            try
            {
                ...
                using (var statement = conn.Prepare(sql))
                {
                    ...
                    {
                        alertStr.Append("Title: ");
                        alertStr.Append((string)statement[0]);
                        alertStr.Append(" Description: ");
                        alertStr.Append((string)statement[1]);
                        alertStr.Append(" Time: ");
                        alertStr.Append((string)statement[2]);
                        alertStr.Append("\n");
                    }

                    if (alertStr.Equals(new StringBuilder()))
                        alertStr.Append("No corresponding result!\n");
                    await new MessageDialog(alertStr.ToString()).ShowAsync();
                    search.Text = "";
                }
            }
            catch (Exception)
